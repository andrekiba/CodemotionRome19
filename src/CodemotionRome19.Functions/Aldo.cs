...
namespace CodemotionRome19.Functions
{
    public class Aldo
    {
        ...
        {            
            var reprompt = new Repr($"Ad esempio puoi dirmi, {S.BreakStrong} Crea una Function App. Oppure, {S.BreakStrong} Deploya il progetto Super Segreto.".ToSsmlSpeech());
        ...
        {
            var response = ResponseBuilder.Tell($"Ad esempio prova a dirmi {S.BreakStrong} Crea un App Service. Oppure, {S.BreakStrong} Fai la build del progetto Round.".ToSsmlSpeech());
            response.Response.ShouldEndSession = false;
            return response;
        }
        ...
        {
            var reprompt = new Repr("Desideri ancora creare un servizio su Azure o fare il deploy di un progetto?");
        ...
        static SkillResponse HandleSetResourceTypeIntent(IntentRequest request, Session session)
        {
            ...
            if (!arType.TryParseAzureResourceType(out var azureResourceType))
            {
                ...
                response = ResponseBuilder.Ask(("Non ho capito che tipo di risorsa vuoi creare. " +
                                               ...
                                               $"{S.BreakStrong} Per favore, {S.BreakStrong} puoi dirmelo di nuovo?").ToSsmlSpeech(), reprompt);
            }
            else
            {
        ...
        static SkillResponse HandleAskForResourceNameIntent(IntentRequest request, Session session)
        {
            ...
            if (request.Intent.Name == Intents.YesIntent)
            {
                ...
                reprompt = new Repr("Come la devo chiamare?");
            ...
            else
            {
                ...
                response = ResponseBuilder.Ask($"Sto per creare la risorsa {S.BreakMedium} {azureResourceType.Name}. Confermi?".ToSsmlSpeech(), reprompt, session);                
            }
        ...
        static SkillResponse HandlSetResourceNameIntent(IntentRequest request, Session session)
        {
            ...
            var response = ResponseBuilder.Ask($"Sto per creare la risorsa {S.BreakMedium} {azureResourceType.Name}, con il nome {arName}. Confermi?".ToSsmlSpeech(), reprompt, session);           
            return response;
        }
            ...
            IAsyncCollector<AzureResourceToDeploy> resourceDeployQueue)
        {
            ...
            if (request.Intent.Name == Intents.YesIntent)
            {
                ...
                if (azureResourceType.Id == AzureResourceTypes.Functions.Id)
                {
                    ...
                    response = ResponseBuilder.Ask($"Bene! {S.BreakStrong} Vuoi anche fare il deploy di un progetto sulla tua nuova risorsa?".ToSsmlSpeech(), reprompt, session);
                }
                else
                {
            ...
            IAsyncCollector<AzureResourceToDeploy> resourceDeployQueue)
        {
            ...
            if (request.Intent.Name == Intents.YesIntent)
            {
                ...
                response = ResponseBuilder.Ask($"Molto bene! {S.BreakStrong} Come si chiama il progetto?".ToSsmlSpeech(), reprompt, session);
            }
            else
            {
                response = await StartResourceCreation(session, resourceDeployQueue);
            ...
            IAsyncCollector<AzureResourceToDeploy> resourceDeployQueue)
        {
            ...
            else if(request.DialogState == States.DialogComplete && request.Intent.ConfirmationStatus == States.IntentConfirmed)
            {
                ...
                if (session.Attributes != null && session.Attributes.ContainsKey(Slots.AzureResourceType))
                {
                    ...
                    var message = ($"OK, creo la risorsa {S.Break} {arName ?? azureResourceType.Name} " +
                                  $"e deployo il progetto {S.Break} {projectSlot.Value}. {S.Notify}").ToSsmlSpeech();

                    session.Attributes["state"] = States.AskForAnotherResource;
                    var reprompt = new Repr("Vuoi creare un'altra risorsa su Azure?");

                    response = ResponseBuilder.Ask(message, reprompt, session);
                    await resourceDeployQueue.AddAsync(ard);
                }
                else
                {
                    var reprompt = new Repr("Vuoi deployare un altro progetto?");
                    response = ResponseBuilder.Ask($"OK, deployo il progetto {S.BreakMedium} {projectSlot.Value}. {S.Notify}".ToSsmlSpeech(), reprompt, session);
        ...
        {
            return request.Intent.Name == Intents.BuildProjectIntent;
        }

        static async Task<SkillResponse> HandleBuildProjectIntent(IntentRequest request, Session session,
            IAsyncCollector<ProjectToBuild> projectBuildQueue)
        {
            ...
            else if (request.DialogState == States.DialogComplete && request.Intent.ConfirmationStatus == States.IntentConfirmed)
            {
                ...
                var reprompt = new Repr("Vuoi fare la build di un altro progetto?");
                response = ResponseBuilder.Ask($"OK, faccio la build del progetto {S.BreakMedium} {projectSlot.Value}. {S.Notify}".ToSsmlSpeech(), reprompt, session);
        ...
        {
            var reprompt = new Repr($"Se hai bisogno di aiuto prova a dire Aiuto");
            var response = ResponseBuilder.Ask($"Scusa ma non ho capito cosa mi hai chiesto. {S.BreakStrong} Per favore, dimmelo di nuovo.".ToSsmlSpeech(), reprompt);
            return response;
        }
        ...
        static async Task<SkillResponse> StartResourceCreation(Session session, IAsyncCollector<AzureResourceToDeploy> resourceDeployQueue)
        {
            ...
            var message = arName is null ? $"OK, creo la risorsa {S.Break} {azureResourceType.Name}. {S.BreakStrong} {S.Notify}".ToSsmlSpeech()
                : $"OK, creo la risorsa {S.Break} {arName}. {S.BreakStrong} {S.Notify}".ToSsmlSpeech();
